...
export default function SchoolLinkRequests() {
  ...
  const handleApproveReg = async (req: any) => {
    ...
    // Create the school_user entry (user_code auto-generated by trigger)
    const { data: insertedUser, error: insertError } = await supabase.from("school_users").insert({
      ...
      id_passport_number: (req as any).id_passport_number || null,
      is_active: true,
    ...
    // Send User ID notification via edge function
    if (insertedUser?.id) {
      try {
        await supabase.functions.invoke("notify-portal-approval", {
          body: { school_user_id: insertedUser.id },
        });
      } catch (err) {
        console.error("Failed to send User ID notification:", err);
      }
    }

    toast({ title: "Registration approved", description: `${req.full_name} can now access the portal. User ID has been sent.` });
    fetchRegRequests();
  };
