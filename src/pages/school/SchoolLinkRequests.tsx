...
export default function SchoolLinkRequests() {
  ...
  const handleApproveReg = async (req: any) => {
    ...
    // Find the existing school_user by auth_user_id and link them to this school
    if (req.auth_user_id) {
    ...
    toast({ title: "School link approved", description: `${req.full_name} is now linked to your school.` });
    fetchRegRequests();
  };
                    ...
                    <TableBody>
                      {regRequests.map(r => (
                        <TableRow key={r.id}>
                          ...
                          <TableCell>{(r as any).id_passport_number || r.student_id_number || "—"}</TableCell>
                        ...
                        <TableRow key={r.id}>
                          ...
                          <TableCell>
                            <div className="flex gap-1">
                              <Badge variant={r.parent_approved ? "default" : "outline"} className="text-xs">
                                Parent {r.parent_approved ? "✓" : "✗"}
                              </Badge>
                              <Badge variant={r.student_approved ? "default" : "outline"} className="text-xs">
                                Student {r.student_approved ? "✓" : "✗"}
                              </Badge>
                            </div>
                          </TableCell>
                          <TableCell><Badge variant={statusColor(r.status) as any} className="capitalize">{r.status}</Badge></TableCell>
