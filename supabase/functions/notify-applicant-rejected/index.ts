...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    await logEmailSend(supabase, { recipientEmail: applicantEmail, subject: learnerSubject, success: learnerEmailResult.success, resendId: learnerEmailResult.resendId, errorMessage: learnerEmailResult.error, templateKey: "applicant-rejected", relatedTable: "applications" });
    ...
    if (parentEmailsEnabled && parentEmail && parentEmail.trim() !== "" && parentEmail !== applicantEmail) {
      ...
      await logEmailSend(supabase, { recipientEmail: parentEmail, subject: parentSubject, success: parentEmailResult.success, resendId: parentEmailResult.resendId, errorMessage: parentEmailResult.error, templateKey: "parent-rejected", relatedTable: "applications" });
    }
